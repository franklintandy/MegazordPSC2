...
import variaveis2 as variaveis
            ...
            #decisão de alvo
                ...
                if variaveis.gameState == variaveis.atacarState:
                    ...
                    if targetsEnemyCanAttack:
                        warrior.attack(targetsEnemyCanAttack.closest_to(warrior))
                    elif targets:
                ...
                if variaveis.forte2 != "":
                    ...
                    safePlace = (self.townhalls | self.structures(UnitTypeId.PYLON))
        ...
        #WARRIORS
         ...
         #train
            ...
            for gate in self.structures(UnitTypeId.GATEWAY).ready.idle:
                if self.structures(UnitTypeId.PYLON).closer_than(8, gate).amount<1:
                    print("construindo pylon no gate")            
                    await self.build(UnitTypeId.PYLON, near=gate)
...
##########################################
    ...
    async def Status_machine(self, iteration): 
        if iteration == 0 or (self.workers.amount/20 < self.townhalls.amount and self.idle_worker_count>6) and variaveis.late_game < 0:
        ...
        elif self.supply_army + self.supply_workers >= 190 and variaveis.gameState <= variaveis.fortificarState and self.supply_army + self.supply_workers < 197:
        ...
        elif self.supply_army + self.supply_workers >= 197:
            if variaveis.gameState != variaveis.atacarState:
                await self.chat_send("gameState(atacarState)")
            variaveis.gameState = variaveis.atacarState
        elif self.supply_army + self.supply_workers >160:
            variaveis.gameState = variaveis.manterState
...
#STRUCTURES
            ...
            if self.townhalls.amount > 0:
                ...
                if self.townhalls.ready.amount + self.already_pending(UnitTypeId.NEXUS) <= variaveis.limiteTowns:      
                    if self.can_afford(UnitTypeId.NEXUS):
                        await self.expand_now()
                    for nexusx in self.townhalls:
                        if self.already_pending(UnitTypeId.NEXUS):
                            for nextnexus in self.townhalls:
                                tupla = (nexusx.position, nextnexus.position)
                                tupla2 = (nextnexus.position, nexusx.position)
                                if nexusx.position.distance_to(nextnexus.position) < 10 and nexusx.position.distance_to(nextnexus.position) != 0 and tupla not in variaveis.tuplesNexusCounted and tupla2 not in variaveis.tuplesNexusCounted:
                                    variaveis.limiteTowns+=1
                                    variaveis.tuplesNexusCounted.append(tupla)
                                    variaveis.tuplesNexusCounted.append(tupla2)
                                    print("tuplas: ", variaveis.tuplesNexusCounted, variaveis.limiteTowns)
                                    if self.can_afford(UnitTypeId.NEXUS):
                                        await self.expand_now()
                                        break
        ...
        if self.time > 900 and variaveis.late_game < 0:
            print("entrando nos 5min de jogo", self.time)
            variaveis.late_game += 1
        if self.time > 1200 and variaveis.late_game < 1:
            print("entrando no late_game", self.time)
            variaveis.limiteTowns += 1
            variaveis.late_game += 1
        if self.time > 1500 and variaveis.late_game < 2:
            print("entrando no late_game2", self.time)
            variaveis.limiteTowns += 1
            variaveis.limiteGeyser += 4
            variaveis.late_game += 1
        if self.time > 1800 and variaveis.late_game < 3:
            print("entrando no late_game3", self.time)
            variaveis.limiteTowns = 16
            variaveis.limiteGeyser += 2
            variaveis.late_game += 1
        if variaveis.gameState >= variaveis.fortificarState:
...
def main():
    sc2.run_game(
        ...
        [Bot(Race.Protoss, CannonRushBot(), name="Megazord2"), Bot(Race.Protoss, megazord.CannonRushBot(), name="Megazord")],
